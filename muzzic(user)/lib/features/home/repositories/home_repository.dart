...
class HomeRepository {
    ...
    required String token,
      ...
      try {
        final request = http.MultipartRequest(
            'POST',
            Uri.parse('${ServerConstants.serverURL}/user_song/upload')
        );
    ...
    Future<List<SongModel>> getGlobalSongs({
      required String token,
    }) async {
      ...
      final decode = jsonDecode(res.body) as List<dynamic>;
      if (decode == null || decode.isEmpty) {
        return []; // no recently played songs
      }
      final songs = decode.map((e) => SongModel.fromMap(e as Map<String, dynamic>, isGlobal: true)).toList();
      // songs.sort((a,b));
      return songs;
    }
    ...
    Future<Either<AppFailure, bool>> favSong({
      ...
      try {
        final body = {
          ...
          if (userSongId != null) "user_song_id": userSongId,
        };

        final res = await http.post(Uri.parse('${ServerConstants.serverURL}/user_song/favorite'),
            headers: {
              'Content-Type': 'application/json',
              'x-auth-token': token,
            },
    ...
    Future<Either<AppFailure ,List<SongModel>>> getFavSongs({
      ...
      try {
        final res = await http.get(
          Uri.parse('${ServerConstants.serverURL}/user_song/list/favorite'),
          headers: {
            'Content-Type': 'application/json',
            'x-auth-token': token,
          },
    ...
    Future<List<SongModel>> getSongsList({
      ...
      final response = await http.get(
            Uri.parse('${ServerConstants.serverURL}/user_song/list'),
          headers: {
            'Content-Type': 'application/json',
            'x-auth-token': token
          }
      ...
      final songs = decoded.map((e) => SongModel.fromMap(e as Map<String, dynamic>, isGlobal: false)).toList();
    ...
    Future<List<SongModel>> getUserSongs({
      ...
      final songs = decode.map((e) => SongModel.fromMap(e as Map<String, dynamic>, isGlobal: false)).toList();
      // songs.sort((a,b));
      return songs;
    }
    ...
    Future<Either<AppFailure, String>> editSong({
      ...
      try {
        var uri = Uri.parse('${ServerConstants.serverURL}/user_song/list/$id');
    ...
    Future<String> deleteSong({
      ...
      final response = await http.delete(
        Uri.parse('${ServerConstants.serverURL}/user_song/list/$id'),
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': token
        },
